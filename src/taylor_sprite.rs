// src/taylor_sprite.rs - Taylor Swift sprite system
use raylib::prelude::*;
...
impl TaylorSprite {
    ...
    pub fn update(&mut self, delta_time: f32, distance_to_player: f32) {
        ...
        // Cambiar entre modo normal y amenazante según la distancia
    ...
    pub fn render_3d(
        ...
        if distance < 30.0 || distance > 1000.0 { return; }
        ...
        let wall_height_factor = 1.5; // Taylor es más alta
        ...
        let sprite_width = adjusted_sprite_height * 0.6; // Mantener proporción
        ...
        // Renderizar sprite usando la imagen
        ...
        // Efecto de aura cuando está muy cerca
        if distance < 100.0 {
    ...
    fn render_menacing_aura(
        ...
        // Dibujar aura alrededor del sprite
        let expand = 5;
        for y in (start_y - expand)..(end_y + expand) {
            for x in (start_x - expand)..(end_x + expand) {
                if x >= 0 && x < framebuffer.width as i32 && y >= 0 && y < framebuffer.height as i32 {
                    // Solo dibujar en los bordes del aura
                    if (x < start_x || x >= end_x || y < start_y || y >= end_y) && 
                       (x + y) % 3 == 0 { // Patrón disperso
